...
// ==========================================
// 🚀 3. CONFIGURACIÓN DEL NAVEGADOR (EQUILIBRADO PARA 512MB)
// ==========================================
const client = new Client({
    ...
    puppeteer: {
        ...
        args: [
            ...
            '--disable-gpu',
            '--no-zygote',
            '--single-process' // <- EL SALVAVIDAS: Obliga a Chrome a caber en 512MB
        ],
        timeout: 0
    }
});
    ...
    try {
        ...
        if (sesiones[fone]?.paso === 'menu_admin') {
            if (chat === '1') { sesiones[fone].paso = 'admin_titulo'; msg.reply('🎬 Escribe el Título:'); return; }
                ...
                r.rows.forEach(p => { lista += `ID: ${p.id} | ${p.titulo} (${p.horario}) | Libres: ${p.cupos_disponibles}\n`; });
                msg.reply(lista); delete sesiones[fone]; return;
            }
            else if (chat === '3') {
                ...
                sesiones[fone].paso = 'admin_eliminar_pelicula'; msg.reply(lista); return;
            }
            else if (chat === '4') {
        ...
        if (sesiones[fone]?.paso === 'admin_horario') { sesiones[fone].horario = msg.body; sesiones[fone].paso = 'admin_cupos'; msg.reply('🪑 Cupos totales en sala:'); return; }
        if (sesiones[fone]?.paso === 'admin_cupos') {
            ...
            msg.reply('✅ Película agregada con éxito a la base de datos.'); delete sesiones[fone]; return;
        }
        ...
        if (sesiones[fone]?.paso === 'admin_eliminar_pelicula') {
            ...
            msg.reply('✅ Película y sus reservas eliminadas del sistema.'); delete sesiones[fone]; return;
        }
        ...
        if (chat.startsWith('*validar ')) {
            ...
            if (check.rows.length === 0) return msg.reply(`⚠️ Alerta: No existe el ticket #${id}.`);
            if (check.rows[0].asistio) return msg.reply(`⚠️ CUIDADO: El Ticket #${id} YA FUE USADO ANTERIORMENTE.`);
            ...
            client.sendMessage(`${check.rows[0].telefono_cliente}@c.us`, `🎟️ ¡Hola ${check.rows[0].nombre_cliente}! Disfruta la película en *La Fábrica de los Sueños*. ¡No olvides pasar por la dulcería! 🍿`);
            return;
        }
        ...
        if (chat === '1') {
            ...
            if (res.rows.length === 0) return msg.reply('Lo sentimos, no hay funciones disponibles en este momento. 😔');
            let listaPelis = '🎬 *Cartelera Disponible:*\n\n'; 
            res.rows.forEach(p => { listaPelis += `ID: *${p.id}* - ${p.titulo} (${p.horario})\n`; });
            msg.reply(listaPelis + '\n👉 *Envía solo el número de ID* de la película que deseas ver:');
            sesiones[fone] = { paso: 'eligiendo_pelicula' }; return;
        } 

        if (sesiones[fone]?.paso === 'eligiendo_pelicula') { sesiones[fone] = { paso: 'esperando_nombre', peliculaId: chat }; msg.reply('Excelente elección. ¿A nombre de quién hacemos la reserva? (Nombre y Apellido)'); return; }
        if (sesiones[fone]?.paso === 'esperando_nombre') { sesiones[fone].nombre = msg.body; sesiones[fone].paso = 'eligiendo_cantidad'; msg.reply('¿Cuántos espacios necesitas en total? (Envía solo el número)'); return; }
        ...
        if (chat === '2') { msg.reply('🎥 Nuestra entrada es totalmente gratis. El proyecto cultural se sostiene gracias a nuestra Dulcería 🍿. ¡Llega temprano, compra tus snacks y apóyanos!'); } 
